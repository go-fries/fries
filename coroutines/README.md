...
`coroutines` provides small, context-aware helpers for concurrent batch work.
...
```bash
go get github.com/go-fries/fries/coroutines/v4
```

## Run tasks

```go
err := coroutines.RunLimit(ctx, 4,
...
```go
err := coroutines.ForEach(ctx, 8, userIDs, func(ctx context.Context, id int64) error {
...
```go
profiles, err := coroutines.Map(ctx, 8, userIDs,
