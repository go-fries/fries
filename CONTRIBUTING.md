...
| --- | --- | --- | --- |
| [Cache](cache/README.md#recommended-usage) | Backend constructor, then `cache.NewRepository(store)` and `cache.Typed[T](repository)` for a reusable typed view | Typed view's `Get`, `Set`, `Remember`; `Repository.Delete`, `Repository.Has` | Views require only `Get/Put`; typed `Set` reports unconfirmed writes as errors. One-off `cache.Get[T]`, `cache.Remember` and existing Repository operations remain available. |
...
| [Queue](queue/README.md#recommended-usage) | Shared `queue.Define[T](name, opts...)`, backend constructor, then `queue.NewProducer` and `queue.NewWorker` | Definition's `Enqueue`, `Handle` and `HandleFor`; `Tasker` / `HandleTasker` for an object-owned API | Definitions share task names, payload types and codecs without consumer dependencies. Set `queue.WithCodec` on the definition for custom encoding. Existing typed helpers and raw `Producer.Enqueue` / `Handle` remain available. |
