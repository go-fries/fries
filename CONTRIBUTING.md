...
| [Queue](queue/README.md#recommended-usage) | Shared `queue.Define[T](name)`, backend constructor, then `queue.NewProducer` and `queue.NewWorker` | Definition's `Enqueue`, `Handle` and `HandleFor`; `Tasker` / `HandleTasker` for an object-owned API | Definitions share task names and payload types without consumer dependencies. Use their `WithCodec` variants for custom codecs. Existing typed helpers and raw `Producer.Enqueue` / `Handle` remain available. |
