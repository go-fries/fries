module github.com/go-fries/fries/cache/redis/v3

go 1.23.0

replace (
	github.com/go-fries/fries/cache/v3 => ../
	github.com/go-fries/fries/codec/json/v3 => ../../codec/json
	github.com/go-fries/fries/codec/v3 => ../../codec
	github.com/go-fries/fries/locker/redis/v3 => ../../locker/redis
	github.com/go-fries/fries/locker/v3 => ../../locker
)

require (
	github.com/go-fries/fries/cache/v3 v3.6.1
	github.com/go-fries/fries/codec/json/v3 v3.6.1
	github.com/go-fries/fries/codec/v3 v3.6.1
	github.com/go-fries/fries/locker/redis/v3 v3.6.1
	github.com/go-fries/fries/locker/v3 v3.6.1
	github.com/redis/go-redis/v9 v9.10.0
	github.com/stretchr/testify v1.10.0
)

require (
	github.com/cespare/xxhash/v2 v2.3.0 // indirect
	github.com/davecgh/go-spew v1.1.1 // indirect
	github.com/dgryski/go-rendezvous v0.0.0-20200823014737-9f7001d12a5f // indirect
	github.com/google/uuid v1.6.0 // indirect
	github.com/pmezard/go-difflib v1.0.0 // indirect
	gopkg.in/yaml.v3 v3.0.1 // indirect
)
